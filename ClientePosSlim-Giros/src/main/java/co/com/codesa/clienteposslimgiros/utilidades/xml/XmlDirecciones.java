...
public class XmlDirecciones{
	
	private static XmlDirecciones instancia;
	...
	private XmlDirecciones()throws Exception{
		
		super();
		
		this.strNombreArchivo = EnumParametrosApp.RUTA_INTERNA_XML_DIRECCIONES.getValorPropiedad();
	...
	public static XmlDirecciones getInstancia()throws Exception{
		
		if (instancia == null){
			instancia = new XmlDirecciones();
		}
	...
	private void cargarEntidades()throws Exception{
		...
		if (this.objDocument != null){
			...
			for (int id = 0; id < lstNodosHijos.getLength(); id++){
				
				objNodoEntidad = lstNodosHijos.item(id);
	...
	public static void main(String args[]){
		...
		try{
			...
			listado = XmlDirecciones.getInstancia().getHmEntidades();
		
			for(Entidad entidad : listado.values()){
				System.out.println(entidad);
			}
			
			entidadSeleccionado = XmlDirecciones.getInstancia().obtenerEntidad("D4");
